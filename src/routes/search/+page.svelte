...
<svelte:head>
  ...
  <meta
    name="description"
    content="Search for expert witnesses by specialty, keyword, or category. Find qualified experts for your legal case."
  />
</svelte:head>
      ...
      <section>
        ...
        >
          ...
          <p class="m-0 shrink-0 text-sm text-muted-foreground tabular-nums">
            {searchResults.length} matching {searchResults.length === 1
              ? "specialty"
              : "specialties"}
          </p>
        </div>
        <div class="flex flex-col gap-2">
          {#each searchResults.slice(0, 50) as { subcategory, category, categorySlug }}
            <a
              href="/categories/{categorySlug}?sub={encodeURIComponent(
                subcategory,
              )}"
    ...
    {:else}
      <section class="py-12 text-center">
        ...
        <p class="mb-8 text-muted-foreground">
          Try broadening your search or browse by category:
        </p>
        <div class="flex flex-wrap justify-center gap-4">
          {#each filteredCategories.slice(0, 6) as cat}
            <Button href="/categories/{cat.slug}" variant="secondary"
              >{cat.name}</Button
            >
          {/each}
        </div>
